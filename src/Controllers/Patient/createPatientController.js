...
export const createPatientController = async (request, response) => {
    ...
    const cantidad = await Patient.count();
    if (cantidad > 0) {
        const existente = await Patient.findOne({
            where: { ci_hash: ci_hashed }
        });
        if (existente) {
            return response.status(401).json({ message: 'El paciente ya está registrado' });
        }
    }

    const patient = await Patient.create({
        nombre: nombre,
        nombre_hash: nombre_hashed,
        ci: ci,
        ci_hash: ci_hashed,
        fecha_nac: fecha_nac,
        genero: genero,
